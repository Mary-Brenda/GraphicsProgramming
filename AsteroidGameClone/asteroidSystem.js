class AsteroidSystem {
  ...
  constructor(){
    ...
    this.accelerations = [];
    this.diams = [];
    this.startTime = 5000; // 5secs
    this.intervalTime = 45000; // 30secs
    this.spawnLimit = 0.01;
  }

  run(){
    // sends first wave of asteroids within 5 secs of game start.
    setTimeout(() => {
      this.spawn();
    }, this.startTime);

    // sends subsequent waves of asteroids as frequently as time passes.
    setInterval(() => {
      this.spawnLimit += 0.0001;
    }, this.intervalTime);
